      ...
      if (jsonData.length > 0) {
        ...
        setError(
          columnError.length > 0
            ? `Excel file should contain the exact column names: ${columnError.join(
                ", "
              )}`
            : ""
        );
  ...
  const handleImport = async () => {
    try {
      ...
      const response = await axios.post(
        "https://api2.tuplrc-cla.com/api/resources/import",
        { importData, selectedType, username }
      );
      ...
      if (
        response.data.invalidResources.length > 0 &&
        response.data.insertedRecords.length === 0
      ) {
          ...
          {showResults && insertedResources.length > 0 && (
            <div className="import-results">
              ...
              <p className="text-success">
                Successfully imported: {insertedResources.length}{" "}
                {getResourceTypeName()}
