...
const ViewPatron = () => {
  ...
  const circulationHistoryHeader = [
    "Book/s Issued",
    "Borrow Date",
    "Due Date",
    "Return Date",
  ...
  const [circulationHistoryLoading, setCirculationHistoryLoading] =
    useState(false);
  ...
  const getPatron = async () => {
    ...
    axios
      .get(`https://api2.tuplrc-cla.com/api/patron/${id}`)
      .then((response) => {
  ...
  const getLogHistory = async () => {
    ...
    axios
      .get(`https://api2.tuplrc-cla.com/api/patron/log/${id}`)
      .then((response) => {
  ...
  const getCirculationHistory = async () => {
    ...
    axios
      .get(`https://api2.tuplrc-cla.com/api/patron/circulation/${id}`)
      .then((response) => {
  ...
  const exportCirculationHistory = () => {
    ...
    // Generate filename based on patron's last name
    const fileName = `${
      patron?.patron_lname || "Unknown"
    }-circulation_history.xlsx`;
    XLSX.writeFile(workbook, fileName);
  };
  ...
  return (
    <div className="viewpatron-container  bg-light">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <Link to={"/patron"}>
          <button className="view-patron-back-button btn ">Back</button>
        </Link>
        {!patronLoading && (
          ...
          <div className="row">
            ...
            <div className="col-md-3 text-center mb-3 mb-md-0">
              {patronLoading ? (
                ...
                <div
                  className="patron-avatar rounded-circle d-flex justify-content-center align-items-center bg-primary text-white mx-auto"
                  style={{ width: "150px", height: "150px", fontSize: "3rem" }}
                >
                  {patron.patron_fname && patron.patron_lname
                    ? `${patron.patron_fname.charAt(
                        0
                      )}${patron.patron_lname.charAt(0)}`
                    : "??"}
                </div>
              )}
            </div>
            ...
            <div className="col-md-9">
              {patronLoading ? (
                ...
                <>
                  <h1 className="card-title mb-3 fw-bold">
                    {patron.patron_fname} {patron.patron_lname}
                  </h1>
