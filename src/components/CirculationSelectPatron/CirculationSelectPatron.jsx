  ...
  return (
    <div className="circ-select-patron-container bg-light">
      ...
      <div className="back-path">
        ...
        <p>
          Book Circulation / <span>Select patron</span>
        </p>
      </div>
      ...
      <div className="patron-list">
        <p>
          List of registered patrons{" "}
          <span>(patrons can only borrow 1 resource at a time)</span>
        </p>
        ...
        {filteredPatrons.length === 0 ? (
          ...
          currentItems
            .filter(
              (patron) =>
                clickedAction !== "Check Out" || patron.status === "active"
            ) // Filter only active patrons
            .map((patron, index) => {
              const isCheckIn =
                localStorage.getItem("clickedAction") === "Check In";
              return isCheckIn || patron.total_checkouts < 1 ? (
                <Link
                  to={`/circulation/patron/item/${patron.patron_id}`}
                  key={index}
                >
                  <div className="row patron">
                    <div className="col">
                      <input type="radio" /> {patron.tup_id}
                    </div>
                    <div className="col-3 text-start d-flex align-items-center justify-content-center">
                      {patron.patron_fname} {patron.patron_lname}
                    </div>
                    ...
                    <div className="col-3 d-flex align-items-center justify-content-center">
                      {patron.course_name == null
                        ? "No course"
                        : patron.course_name}
                    </div>
                    <div className="col-2 d-flex align-items-center justify-content-center">
                      {patron.total_checkouts}
                    </div>
                  </div>
                </Link>
              ) : (
                <div className="row patron disabled grey" key={index}>
                  <div className="col">
                    <input type="radio" disabled /> {patron.tup_id}
                  </div>
                  <div className="col-3 text-start d-flex align-items-center">
                    {patron.patron_fname} {patron.patron_lname}
                  </div>
