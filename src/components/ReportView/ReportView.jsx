...
const ReportView = ({ open, close, id }) => {
  ...
  const getReport = async () => {
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/reports/view/${id}`
      );
  ...
  const fetchExcel = async () => {
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/reports/fetch-excel?filePath=${encodeURIComponent(
          report[0].filepath
        )}`,
        {
            ...
            {report.map((item, index) => (
              <div className="details" key={item.id || index}>
                <div className="row mt-3">
                  ...
                  <div
                    className={`col-10 fw-semibold ${
                      item.is_archived == 1 ? "text-danger" : "text-success"
                    }`}
                  >
                    {item.is_archived == 1 ? "Archived" : "Unarchived"}
                  </div>
