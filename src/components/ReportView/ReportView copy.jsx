...
const ReportView = ({ open, close, id }) => {
  ...
  const getReport = async () => {
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/reports/${id}`
      );
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.writeFile(
        workbook,
        `${sanitizedReportName}_${dayjs().format("YYYY-MM-DD")}.xlsx`
      );
            ...
            {report.map((item, index) => (
              <div className="details" key={item.id || index}>
                <div className="row">
                  ...
                  {item.cat_name !== "inventory" &&
                    item.cat_name !== "patron" &&
                    item.detail_name !== "most borrowed books" &&
                    item.detail_name !== "least borrowed books" && (
                      <>
                        <div className="col-2 fw-semibold">
                          Report start date:
                        </div>
                        <div className="col-10">
                          {item.report_start_date || "N/A"}
                        </div>
