...
const AdminTopNavbar = () => {
  ...
  useEffect(() => {
    ...
    const allDataLoaded =
      ...
      type.length > 0 &&
      publisher.length > 0 &&
      publisherInfo.length > 0 &&
      author.length > 0 &&
      adviser.length > 0 &&
      department.length > 0 &&
      topic.length > 0;

    if (isOnline === true && allDataLoaded) {
      // Explicitly check isOnline === true
      console.log(
        "Online and all necessary data loaded, refreshing IndexedDB non-resource stores..."
      );
      ...
      refreshNonResourceStores(
        ...
        .then(() => {
          console.log(
            "Successfully refreshed non-resource stores in IndexedDB."
          );
        })
        .catch((error) => {
          console.error(
            "Failed to refresh non-resource stores in IndexedDB:",
            error
          );
      ...
      console.log(
        "Offline. Will rely on existing IndexedDB data (fetched via getOfflineData)."
      );
