...
import {
  getAllFromStore,
  getAllUnsyncedFromStore,
  getBook,
  getBookPub,
  getCatalogDetailsOffline,
  getPub,
  getResource,
  getResourceAdviser,
  getResourceAuthors,
} from "../../indexedDb/getDataOffline";
import {
  clearObjectStore,
  deleteResourceFromIndexedDB,
  markAsSynced,
} from "../../indexedDb/syncData";
...
import {
  setAdvancedSearch,
  setIsSearch,
} from "../../features/advancedSearchSlice";
...
const Catalog = () => {
  ...
  const { advancedSearch, isSearch } = useSelector(
  ...
  useEffect(() => {
    if (catalog.length > 0) {
      ...
      if (sortOrder.author === 1) {
        sortedCatalog.sort((a, b) => {
          const authorA = Array.isArray(a.author_names)
            ? a.author_names[0] || ""
            : a.author_names || "";
          const authorB = Array.isArray(b.author_names)
            ? b.author_names[0] || ""
            : b.author_names || "";
    ...
    try {
      ...
      if (advancedSearch.length == 0 && !isSearch) {
        ...
        const response = await axios.get(
          `https://api2.tuplrc-cla.com/api/catalog`,
          {
            params: {
              ...
              sort: selectedFilters.title
                ? { column: "title", order: selectedFilters.title }
                : selectedFilters.author
                ? { column: "author", order: selectedFilters.author }
                : null,
            },
          }
        );
        ...
        if (keyword?.trim()) {
          ...
          filteredResults = filteredResults.filter((item) => {
            const titleMatch =
              item.resource_title?.toLowerCase().includes(lowerKeyword) ??
              false;
            const authorMatch =
              item.author_names?.toLowerCase().includes(lowerKeyword) ?? false;
    ...
    try {
      ...
      if (keyword !== "") {
        ...
        filteredData = data.filter((item) => {
          ...
          const titleMatch = item.resource_title
            .toLowerCase()
            .includes(keyword.toLowerCase());

          // Check if any author name in the array matches the keyword (case-insensitive)
          const authorMatch =
            Array.isArray(item.author_names) &&
            item.author_names.some((author) =>
      ...
      if (selectedFilters.type) {
        filteredData = filteredData.filter(
          (item) => item.type_id === selectedFilters.type
        );
      }
      if (selectedFilters.department) {
        filteredData = filteredData.filter(
          (item) => item.dept_id === selectedFilters.department
        );
      }
      if (selectedFilters.topic) {
        filteredData = filteredData.filter(
          (item) => item.topic_id === selectedFilters.topic
        );
      }
  ...
  const getType = async () => {
    try {
      const response = await axios
        .get("https://api2.tuplrc-cla.com/api/data/type")
        .then((res) => res.data);
      //console.log(response)
      setType(response);
    } catch (err) {
  ...
  const getDept = async () => {
    try {
      const response = await axios
        .get("https://api2.tuplrc-cla.com/api/data/departments")
        .then((res) => res.data);
      setDepartment(response);
    } catch (err) {
      console.log(
        "Couldn't retrieve department online. An error occurred: ",
        err.message
      );
    }
  };
  ...
  const getTopics = async () => {
    try {
      const response = await axios
        .get("https://api2.tuplrc-cla.com/api/data/topic")
        .then((res) => res.data);
      setTopic(response);
    } catch (err) {
      console.log(
        "Couldn't retrieve topics online. An error occurred: ",
        err.message
      );
    }
  };
      ...
      for (const resource of resources) {
        try {
          ...
          const response = await axios.post(
            "https://api2.tuplrc-cla.com/api/sync/resources",
            resource
          );
          ...
          console.log(
            `Synced resource: ${resource.resource_id}`,
            response.data
          );
          ...
          console.log(
            `Resource ${resource.resource_id} deleted from IndexedDB.`
          );
        } catch (error) {
          setStatusModal(true);
          ...
          console.error(
            `Failed to sync resource: ${resource.resource_id}`,
            error.message
          );
        }
      }
  ...
  const syncAdviserOnline = async (adviser, resourceId) => {
    try {
      ...
      const response = await axios.post(
        "https://api2.tuplrc-cla.com/api/sync/adviser",
        { adviser, resourceId }
      );
  ...
  const syncAuthorsOnline = async (authors, resourceId) => {
    try {
      for (const author of authors) {
        try {
          const response = await axios.post(
            "https://api2.tuplrc-cla.com/api/sync/authors",
            { author, resourceId }
          );
          console.log(`Synced author: ${author.author_id}`, response.data);
        } catch (error) {
          console.error(
            `Failed to sync author: ${author.author_id}`,
            error.message
          );
        }
      }
  ...
  const syncPublisherOnline = async (publisher) => {
    try {
      const response = await axios.post(
        "https://api2.tuplrc-cla.com/api/sync/publisher",
        publisher
      );
  ...
  const syncBookOnline = async (book, resourceId, pubId) => {
    try {
      ...
      const response = await axios.post(
        "https://api2.tuplrc-cla.com/api/sync/book",
        formData,
        {
  ...
  const syncJournalNewsletterOnline = async (jn, resourceId) => {
    try {
      ...
      // Send the FormData to the backend
      const response = await axios.post(
        "https://api2.tuplrc-cla.com/api/sync/journalnewsletter",
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        }
  ...
  const handleArchive = async (id, status) => {
    ...
    try {
      await axios.post(`https://api2.tuplrc-cla.com/api/catalog`, {
        id,
        resourceState,
        username,
      });
      ...
      <div className="add-scan-item">
        <div className="d-flex justify-content-between">
          ...
          <Link to="/catalog/add">
            <button
              type="button"
              className="btn cat-button d-flex align-items-center justify-content-center gap-2 "
            >
      ...
      <div className="search-filter d-flex">
        ...
        <div className="input-group">
          <div>
            ...
            <button
              href=""
              className="m-0 advanced-search fw-semibold"
        ...
        <div className="filters d-flex gap-3">
          ...
          <div className="">
            ...
            <select
              ...
              name="isArchived"
              onChange={(e) =>
                handleSelectedFilter("isArchived", e.target.value)
              }
              className="form-select form-select-sm"
            ...
            <td>
              ...
              {isOnline && (
                <FontAwesomeIcon
                  ...
                  className={`sort-icon ps-2 ${
                    sortOrder.title !== 0 ? "active" : ""
                  }`}
            ...
            <td>
              ...
              {isOnline ? (
                <select
                  ...
                  className="sort"
                  onChange={(e) =>
                    handleSelectedFilter("department", e.target.value)
                  }
                >
            ...
            <td>
              ...
              {isOnline ? (
                <select
                  ...
                  className="sort"
                  onChange={(e) =>
                    handleSelectedFilter("topic", e.target.value)
                  }
                >
          ...
          {loading ? (
            ...
            displayedCatalog.map((item, key) => (
              <tr key={key}>
                ...
                <td>{item.topic_name ? item.topic_name : "N/A"}</td>
                <td>
                  {isOnline
                    ? `${item.resource_quantity}/${item.original_resource_quantity}`
                    : `${item.resource_quantity}`}
                </td>
                {/* <td>
                ...
                <td className="">
                  <button
                    ...
                    title="View Resource"
                    onClick={() =>
                      navigate(`/catalog/view/${item.resource_id}`)
                    }
                  >
                  ...
                  <button
                    ...
                    data-placement="top"
                    title={`${
                      item.resource_is_archived == 0 ? "Archive" : "Unarchive"
                    } Resource`}
                    onClick={() =>
                      handleArchive(item.resource_id, item.resource_is_archived)
                    }
                  >
