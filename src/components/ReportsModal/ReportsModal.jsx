...
const ReportsModal = ({ open, close }) => {
  ...
  useEffect(() => {
    ...
    setFilteredCourses(
      courses.filter((item) => item.college_id == reportData.college)
    );
  ...
  useEffect(() => {
    setReportData((prevdata) => ({
      ...prevdata,
      cat_name:
        categories.find((category) => category.cat_id == reportData.category)
          ?.cat_name || "",
      detail_name:
        details.find((detail) => detail.detail_id == reportData.detail)
          ?.detail_name || "",
    }));
  ...
  const getCategories = async () => {
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/reports/categories`
      );
  ...
  const getDetails = async () => {
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/reports/details`
      );
  ...
  const getCollege = async () => {
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/data/college`
      );
  ...
  const getCourse = async () => {
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/data/course`
      );
  ...
  const validateForm = () => {
    ...
    if (!reportData.endDate && datesRequired) {
      ...
      reportData.endDate &&
      new Date(reportData.endDate) < new Date(reportData.startDate)
    ) {
    ...
    try {
      ...
      if (reportDataToSave.length === 0) {
        const formattedStartDate = reportData.startDate
          ? dayjs(reportData.startDate).format("YYYY-MM-DD")
          : "";
        const formattedEndDate = reportData.endDate
          ? dayjs(reportData.endDate).format("YYYY-MM-DD")
          : "";
        ...
        const response = await axios.get(
          `${API_BASE_URL}/api/reports/generate-report`,
          { params }
        );
        ...
        reportDataToSave = response.data.map((row) => {
          ...
          Object.keys(processedRow).forEach((key) => {
            if (
              ...
              ((processedRow[key].includes("T") &&
                processedRow[key].includes("Z")) ||
                processedRow[key].match(/^\d{4}-\d{2}-\d{2}$/))
            ) {
              processedRow[key] = dayjs
                .utc(processedRow[key])
                .local()
      ...
      formData.append(
        "endDate",
        reportData.endDate ? dayjs(reportData.endDate).format("YYYY-MM-DD") : ""
      );
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/reports/generate-report`,
        {
          params: params,
        }
      );
          ...
          {showReportTable && generatedReport.length > 0 ? (
            ...
            <div className="text-center py-4">
              ...
              <p className="m-0 mt-1 fw-semibold">
                No available data to generate report.
              </p>
