...
const Accounts = () => {
  ...
  const editUserAccount = async (isChangePassword) => {
    ...
    const hasChanges =
      ...
      account.password !== "" ||
      originalAccount.confirmPassword !== "";
    ...
    if (Object.keys(error).length === 0) {
      ...
      try {
        ...
        const response = await axios.put(
          `https://api2.tuplrc-cla.com/api/account/${account.id}`,
          account
        );
        const result2 = await Swal.fire({
          title: "Edited!",
  ...
  const passwordValidation = () => {
    ...
    if (!account.password) err.password = "Enter a password";
      ...
      err.password =
        "Password must have one uppercase, one lowercase, one digit, and one special character";
  ...
  const deactivateUser = async (uname, id) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://api2.tuplrc-cla.com/api/account/deactivate/${id}`,
        { username }
      );
      const result2 = await Swal.fire({
        title: "Deactivated!",
  ...
  const activateUser = async (uname, id) => {
    ...
    try {
      const response = await axios.put(
        `https://api2.tuplrc-cla.com/api/account/activate/${id}`,
        { username }
      );
      const result2 = await Swal.fire({
        title: "Activated!",
  ...
  useEffect(() => {
    ...
    if (sortStates.role_name) {
      filteredResults = filteredResults.filter(
        (item) =>
          item.role_name.toLowerCase() === sortStates.role_name.toLowerCase()
      );
    }
    ...
    if (sortStates.staff_status) {
      filteredResults = filteredResults.filter(
        (item) =>
          item.staff_status.toLowerCase() ===
          sortStates.staff_status.toLowerCase()
      );
    }
    ...
    <div className="accounts-container bg-light">
      ...
      <div className="search-add">
        ...
        <div className="input-group w-50 z-0">
          ...
          />
          <button
            className="btn search-btn px-3 shadow-sm"
        ...
        <thead>
          <tr>
            <td>
              ...
              <span
                className="sort-icon"
                onClick={() => handleSortClick("staff_fname")}
              >
            ...
            <td>
              ...
              <span
                className="sort-icon"
                onClick={() => handleSortClick("staff_lname")}
              >
            ...
            <td>
              ...
              <span
                className="sort-icon"
                onClick={() => handleSortClick("staff_uname")}
              >
                    ...
                    <span
                      className={
                        item.staff_status == "active"
                          ? "bg-success text-light p-2 rounded fw-semibold"
                          : "bg-danger text-light p-2 rounded fw-semibold"
                      }
                    >
                      {item.staff_status}
                    </span>
                  ...
                  <td className="action">
                    ...
                    {item.staff_status === "active" ? (
                      <button
                        ...
                        onClick={() =>
                          deactivateUser(item.staff_uname, item.staff_id)
                        }
                      ...
                      <button
                        className="btn deac-acc-btn"
                        onClick={() =>
                          activateUser(item.staff_uname, item.staff_id)
                        }
                        title="Activate user"
                      >
              ...
              <tr>
                <td colSpan="6" className="no-data-box text-center">
                  <div className="d-flex flex-column align-items-center gap-2 my-5">
                    ...
                    <button className="btn btn-warning" onClick={clearFilter}>
                      Clear Filter
                    </button>
                  </div>
                </td>
