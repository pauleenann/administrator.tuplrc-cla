...
const AddItem = () => {
  ...
  const viewResourceOnline = async () => {
    ...
    try {
      const response = await axios.get(
        `https://api2.tuplrc-cla.com/api/resources/${id}`
      );
  ...
  const formValidation = () => {
    ...
    if (
      ...
      bookData.mediaType === "2" ||
      bookData.mediaType === "3"
    ) {
      if (!bookData.topic) {
  ...
  const handleSaveResourceOffline = async () => {
    if (formValidation() === true) {
      ...
      try {
        ...
        window.toast.fire({
          icon: "error",
          title: "Cannot save resource offline",
        });
      }
    } else {
      window.toast.fire({
  ...
  const handleEditResourceOffline = async () => {
    ...
    try {
      ...
      window.toast.fire({
        icon: "error",
        title: "Cannot edit resource offline",
      });
    }
  };
  ...
  const getAuthors = async () => {
    ...
    try {
      const response = await axios.get(
        "https://api2.tuplrc-cla.com/api/data/authors"
      );
      response.data.forEach((item) => {
        auth.push({
  ...
  const getAdvisers = async () => {
    ...
    try {
      const response = await axios.get(
        "https://api2.tuplrc-cla.com/api/data/advisers"
      );
      response.data.forEach((item) => {
        adv.push({
  ...
  const getStatus = async () => {
    try {
      const response = await axios
        .get("https://api2.tuplrc-cla.com/api/data/status")
        .then((res) => res.data);
      //console.log(response)
  ...
  return (
    <div className="add-item-container bg-light">
      ...
      <div className="add-item-path-button">
        ...
        <button
          onClick={() => navigate(-1)}
          className="btn add-item-back-button"
        >
          Back
        </button>
        {/* </Link> */}
        <div className="add-item-path">
          <p>
            Cataloging /{" "}
            <span>
              {disabled ? "View" : editMode ? "Edit" : "Add new"} Item
            </span>
          </p>
        </div>
      </div>
      ...
      {disabled ? (
        ...
        <div className="cancel-save">
          <button className="btn add-item-cancel">Cancel</button>
          <button
            className="btn add-item-save"
            onClick={() => {
