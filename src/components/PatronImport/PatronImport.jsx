  ...
  const handleFileUpload = async (event) => {
    ...
    reader.onload = async (e) => {
      ...
      if (jsonData.length > 0) {
        ...
        columnNames = Object.keys(jsonData[0]).map((key) =>
        ...
        setError(
          columnError.length > 0
            ? `Excel file should contain the following column names: ${columnError.join(
                ", "
              )}`
            : ""
        );
  ...
  const handleImport = async () => {
    try {
      ...
      const response = await axios.post(
        "https://api2.tuplrc-cla.com/api/patron/import",
        { patrons: importData, username, username }
      );
      ...
      if (
        response.data.invalidPatrons.length > 0 &&
        response.data.insertedPatrons.length === 0
      ) {
          ...
          {showResults && insertedPatrons.length > 0 && (
            <div className="import-results">
              ...
              <p className="text-success">
                Successfully imported: {insertedPatrons.length} records
              </p>

              {invalidPatrons.length > 0 && (
                <div className="mt-2">
                  <p className="text-warning">
                    Failed to import: {invalidPatrons.length} records
                  </p>
                  <button
